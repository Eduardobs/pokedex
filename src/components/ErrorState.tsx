...
export function ErrorState({ title, message, titleKey, messageKey, retry, home = false }: Props) {
  ...
  return (
    <section className="state-page" role="alert">
      ...
      <h1>{title ?? t(titleKey ?? 'error.title')}</h1>
      <p>{message ?? t(messageKey ?? 'error.message')}</p>
      <div className="state-actions">
        ...
        <Link to={home ? '/' : '/pokemon'} className="button secondary">
          <ArrowLeft size={18} /> {t(home ? 'error.home' : 'error.back')}
        </Link>
      </div>
    </section>
  )
