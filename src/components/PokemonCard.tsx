...
export function PokemonCard({ id, name, pokemon, sortMetric }: Props) {
  ...
  const normalArtwork = detail?.sprites.other?.['official-artwork']?.front_default
    ...
    <article ref={cardRef} className="pokemon-card">
      ...
      <div className="pokemon-image-wrap">
        <Link to={`/pokemon/${name}`} className="pokemon-image-link">
          ...
          <img src={artwork} alt={`${displayName} — ${t(shiny ? 'detail.shiny' : 'detail.normal')}`} width="165" height="165" loading="lazy" decoding="async" />
        </Link>
        <button
          type="button"
          className={`card-shiny-toggle ${shiny ? 'active' : ''}`}
          aria-label={t(shiny ? 'pokemonCard.showNormal' : 'pokemonCard.showShiny', { name: displayName })}
          aria-pressed={shiny}
