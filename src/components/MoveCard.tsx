...
export function MoveCard({ move, method, level }: Props) {
  ...
  const learning = method === 'level-up' ? `Nível ${level}` : prettyName(method)
  return (
    <Link ref={cardRef} className={`move-card ${data ? `damage-border-${data.damage_class.name}` : ''}`} to={`/explorar/move/${move.name}`}>
      ...
      {data && <div className="move-numbers"><span>Poder <b>{data.power ?? '—'}</b></span><span>Precisão <b>{data.accuracy ?? '—'}</b></span></div>}
    </Link>
  )
}
