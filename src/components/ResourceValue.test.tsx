...
function renderValue(value: unknown, depth = 0) {
  return render(
    <MemoryRouter>
      <LanguageProvider>
        <ResourceValue value={value} depth={depth} />
      </LanguageProvider>
    </MemoryRouter>,
  )
}
...
describe('ResourceValue', () => {
  ...
  it('shows Pokémon arrays as paginated visual cards with artwork, name, and Pokédex number', () => {
    ...
    expect(screen.getByRole('link', { name: 'Abrir Pokemon 1, Pokémon número 1' })).toHaveAttribute(
      'href',
      '/pokemon/pokemon-1',
    )
  ...
  it('keeps related data visible for nested Pokémon records', () => {
    ...
    expect(screen.getByRole('link', { name: 'Abrir Murkrow, Pokémon número 198' })).toHaveAttribute(
      'href',
      '/pokemon/murkrow',
    )
