...
const resource = (endpoint: string, id: number, name: string): NamedResource => ({ name, url: `https://pokeapi.co/api/v2/${endpoint}/${id}/` })
...
const encounters: Encounter[] = [
  {
    ...
    version_details: [{
      ...
      max_chance: 15,
      encounter_details: [{ min_level: 3, max_level: 5, chance: 10, method: resource('encounter-method', 1, 'walk'), condition_values: [resource('encounter-condition-value', 4, 'time-night')] }],
    }],
  },
  {
    ...
    version_details: [{
      ...
      max_chance: 25,
      encounter_details: [{ min_level: 20, max_level: 20, chance: 25, method: resource('encounter-method', 1, 'walk'), condition_values: [] }],
    }],
  },
]
...
describe('PokemonEncounters', () => {
  it('mostra a versão mais recente e permite consultar outra versão com seus detalhes', () => {
    render(<MemoryRouter><LanguageProvider><PokemonEncounters encounters={encounters} /></LanguageProvider></MemoryRouter>)
    ...
    expect(screen.getByRole('link', { name: 'Abrir dados completos do local' })).toHaveAttribute('href', '/explorar/location-area/viridian-forest-area')
  })
})
