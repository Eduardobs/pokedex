    ...
    <>
      ...
      <p className="encounter-status" role="status">
        {areas.length === 1
          ...
          : t('detail.encounterSummary', {
              count: formatNumber(areas.length, language),
        ...
        <div className="encounter-list">
          {areas.map((area) => (
            <details className="encounter-card" key={area.location_area.name}>
              <summary>
                <span className="encounter-location-icon">
                  <MapPin aria-hidden="true" />
                </span>
                <span className="encounter-location-copy">
                  <b>{prettyName(area.location_area.name)}</b>
                  <small>{t('detail.encounterDetails')}</small>
                </span>
                <span className="encounter-max-chance">
                  {t('detail.bestEncounterChance', {
                    chance: Math.max(...area.encounter_details.map(({ chance }) => chance), 0),
                  })}
                </span>
              ...
              <div className="encounter-card-body">
                <ul className="encounter-detail-list">
                  {area.encounter_details.map((detail, index) => {
                    const level =
                      detail.min_level === 0 && detail.max_level === 0
                        ? t('detail.levelUnavailable')
                        : detail.min_level === detail.max_level
                          ? t('move.level', { level: detail.min_level })
                          : t('detail.levelRange', {
                              minimum: detail.min_level,
                              maximum: detail.max_level,
                            })
                    return (
                      <li
                        key={`${detail.method.name}-${detail.min_level}-${detail.max_level}-${detail.chance}-${index}`}
                      >
                        <dl>
                          <div>
                            <dt>{t('detail.encounterMethod')}</dt>
                            <dd>{localizedApiTerm(detail.method.name, language)}</dd>
                          </div>
                          <div>
                            <dt>{t('detail.encounterLevel')}</dt>
                            <dd>{level}</dd>
                          </div>
                          <div>
                            <dt>{t('detail.encounterChance')}</dt>
                            <dd>{formatNumber(detail.chance, language)}%</dd>
                          </div>
                        </dl>
                        <div className="encounter-conditions">
                          ...
                          {detail.condition_values.length ? (
                            <div>
                              {detail.condition_values.map(({ name }) => (
                                <span key={name}>{localizedApiTerm(name, language)}</span>
                              ))}
                            </div>
                          ) : (
