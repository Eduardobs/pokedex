...
export function evolutionCondition(detail: EvolutionDetail, t: Translate, language: Language = 'pt-BR') {
  ...
  if (detail.used_move)
    conditions.push(
      presentNumber(detail.min_move_count)
        ? t('evolution.useMoveTimes', {
            move: prettyName(detail.used_move.name),
            count: number(detail.min_move_count),
          })
  ...
  if (detail.allowed_natures?.length)
    conditions.push(
      t('evolution.natures', {
        natures: detail.allowed_natures.map(({ name }) => prettyName(name)).join(', '),
      }),
    )
  if (detail.condition_expression?.percentage_chance)
    conditions.push(
      t('evolution.specialChance', {
        chance: number(detail.condition_expression.percentage_chance),
      }),
    )
      ...
      <div className="evolution-entry">
        {!root && (
          <span className="evolution-condition">
            ...
            <span className="evolution-condition-options">
              {evolutionConditions(node.evolution_details, t, language).map((condition) => (
        ...
        <Link to={`/pokemon/${encodeURIComponent(pokemonName)}`} className="evolution-pokemon">
          <img
            src={pokemonArtwork(id)}
            alt={prettyName(node.species.name)}
            width="100"
            height="100"
            loading="lazy"
            decoding="async"
      ...
      {node.evolves_to.length > 0 && (
        <ul className={`evolution-children${hasBranches ? ' is-branching' : ''}`}>
          {node.evolves_to.map((child) => (
            <EvolutionTreeNode key={child.species.name} node={child} t={t} language={language} />
          ))}
        </ul>
      )}
    </li>
