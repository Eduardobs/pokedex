...
export function PokemonDataTab({ pokemon, species, speciesLoading }: Props) {
  const { t } = useLanguage()
  const speciesFlag = (value: boolean | undefined) =>
    speciesLoading ? (
    ...
    >
      <article className="info-card technical-card registry-card">
        ...
        <dl>
          <div>
            <dt>ID</dt>
            <dd>#{String(pokemon.id).padStart(4, '0')}</dd>
          </div>
          <div>
            <dt>{t('detail.apiOrder')}</dt>
            <dd>{pokemon.order}</dd>
          </div>
          <div>
            <dt>{t('detail.defaultForm')}</dt>
            <dd>{t(pokemon.is_default ? 'common.yes' : 'common.no')}</dd>
          </div>
          <div>
            <dt>{t('detail.legendary')}</dt>
            <dd>{speciesFlag(species?.is_legendary)}</dd>
          </div>
          <div>
            <dt>{t('detail.mythical')}</dt>
            <dd>{speciesFlag(species?.is_mythical)}</dd>
          </div>
          <div>
            <dt>{t('detail.species')}</dt>
            <dd>
              <ResourceValue value={pokemon.species} />
            </dd>
          </div>
          <div>
            <dt>{t('detail.forms')}</dt>
      ...
      <article className="info-card technical-card">
        ...
        {cries.length ? (
          <div className="cry-list">
            {cries.map((cry) => (
              <label key={cry.label}>
                <span>{cry.label}</span>
                <audio controls preload="none" src={cry.url} />
              </label>
            ))}
          </div>
        ) : (
          <p className="muted">{t('detail.noHistoricalData')}</p>
        )}
      </article>
      ...
      <article className="info-card technical-card sprites-card">
        ...
        <div className="sprite-grid">
          {sprites.map((sprite) => (
            <figure key={sprite.label}>
              <img src={sprite.url} alt={sprite.label} width="96" height="96" loading="lazy" />
      ...
      <article className="info-card technical-card">
        ...
        {pokemon.held_items?.length ? (
          <ResourceValue value={pokemon.held_items} />
        ) : (
          <p className="muted">{t('detail.noHeldItems')}</p>
        )}
      </article>
      ...
      <article className="info-card technical-card">
        <h2>
          <Gamepad2 />
          {t('detail.gameIndices')}
        </h2>
        {pokemon.game_indices.length ? (
          <ResourceValue value={pokemon.game_indices} />
        ) : (
          <p className="muted">{t('detail.noHistoricalData')}</p>
        )}
      </article>
      ...
      <article className="info-card technical-card api-source-card">
        ...
        <a
          ...
          href={`${API_BASE}/pokemon/${encodeURIComponent(pokemon.name)}`}
          target="_blank"
          rel="noopener noreferrer"
        >
          JSON <ExternalLink size={15} />
        </a>
      </article>
    </div>
  )
