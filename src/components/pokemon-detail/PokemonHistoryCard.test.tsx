...
const pokemon = {
  past_stats: [{ generation: resource('generation', 1, 'generation-i'), stats: [{ base_stat: 50, effort: 0, stat: resource('stat', 9, 'special') }] }],
  past_types: [{ generation: resource('generation', 5, 'generation-v'), types: [{ slot: 1, type: resource('type', 1, 'normal') }] }],
  past_abilities: [{ generation: resource('generation', 4, 'generation-iv'), abilities: [{ ability: null, is_hidden: true, slot: 3 }] }],
} as Pokemon

describe('PokemonHistoryCard', () => {
  it('traduz o histórico técnico em uma linha do tempo legível', () => {
    render(<MemoryRouter><LanguageProvider><PokemonHistoryCard pokemon={pokemon} /></LanguageProvider></MemoryRouter>)
