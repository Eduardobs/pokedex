...
export function TrainingBreedingCard({ pokemon, species }: Props) {
  ...
  return (
    <article className="info-card training-card">
      ...
      <dl>
        <div>
          <dt>{t('detail.effortYield')}</dt>
          <dd>{effortYield.join(', ') || '—'}</dd>
        </div>
        <div>
          <dt>{t('detail.hatchCycles')}</dt>
          <dd>
            {t('detail.hatchCyclesValue', {
              count: formatNumber(species.hatch_counter + 1, language),
            })}
          </dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd>{t(species.has_gender_differences ? 'common.yes' : 'common.no')}</dd>
        </div>
        <div>
          <dt>{t('detail.switchableForms')}</dt>
          <dd>{t(species.forms_switchable ? 'common.yes' : 'common.no')}</dd>
        </div>
      </dl>
