...
function routeFromReference(url: string, resourceName: string) {
  try {
    ...
    const [endpoint, name] = new URL(trustedUrl).pathname
      .slice(`${new URL(API_BASE).pathname}/`.length)
      .split('/')
      .filter(Boolean)
...
function referenceMeta(value: unknown, language: Language) {
  ...
  try {
    const trustedUrl = resolveApiUrl(url)
    const [endpoint, identifier] = new URL(trustedUrl).pathname
      .slice(`${new URL(API_BASE).pathname}/`.length)
      .split('/')
      .filter(Boolean)
              ...
              >
                ...
                <span className="related-pokemon-copy">
                  ...
                  {pokemon.details.length > 0 && (
                    <dl className="related-pokemon-details">
                      {pokemon.details.map((detail) => (
                        <div key={detail.key}>
                          <dt>{resourceFieldLabel(detail.key, language)}</dt>
                          <dd>{relatedDatumValue(detail, language, yes, no)}</dd>
                        </div>
                      ))}
                    </dl>
                  )}
...
export function ResourceValue({
  ...
  if (typeof object.name === 'string' && typeof object.url === 'string') {
    ...
    return route ? (
      <Link className="resource-chip" to={route}>
        {localizedApiTerm(object.name, language)} <ChevronRight size={12} />
      </Link>
    ) : (
      <span>{localizedApiTerm(object.name, language)}</span>
    )
  }
