...
const pokemon = (id: number, name: string, stats: Record<string, number>) => ({
  id,
  name,
  stats: Object.entries(stats).map(([statName, base_stat]) => ({ base_stat, effort: 0, stat: { name: statName, url: '' } })),
} as Pokemon)

const details = {
  charizard: pokemon(6, 'charizard', { hp: 78, attack: 84, defense: 78, 'special-attack': 109, 'special-defense': 85, speed: 100 }),
  bulbasaur: pokemon(1, 'bulbasaur', { hp: 45, attack: 49, defense: 49, 'special-attack': 65, 'special-defense': 65, speed: 45 }),
  blastoise: pokemon(9, 'blastoise', { hp: 79, attack: 83, defense: 100, 'special-attack': 85, 'special-defense': 105, speed: 78 }),
}
