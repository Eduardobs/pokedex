...
function relations({ double = [], half = [], none = [] }: { double?: string[]; half?: string[]; none?: string[] }): TypeDamageRelations {
  return {
...
describe('calculateWeaknesses', () => {
  ...
  it('removes weaknesses neutralized by resistance or immunity', () => {
    const dark = relations({ double: ['fighting', 'bug', 'fairy'], half: ['ghost', 'dark'], none: ['psychic'] })
    const ghost = relations({ double: ['ghost', 'dark'], half: ['poison', 'bug'], none: ['normal', 'fighting'] })

    expect(calculateWeaknesses([dark, ghost])).toEqual([
      { type: 'fairy', multiplier: 2 },
    ])
  })

  it('returns type immunities with a zero multiplier', () => {
    const steel = relations({ double: ['fire', 'fighting', 'ground'], half: ['normal', 'grass'], none: ['poison'] })

    expect(calculateImmunities([steel])).toEqual([
