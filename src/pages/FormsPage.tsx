  ...
  return (
    <section className="page content-width forms-directory-page">
      ...
      <div className="forms-directory-toolbar">
        <div className="forms-category-tabs" role="group" aria-label={t('forms.categoryFilter')}>{categories.map((item) => { const Icon = item.icon; const count = item.value === 'all' ? specialForms.length : grouped[item.value].length; return <button type="button" aria-pressed={category === item.value} className={category === item.value ? `active tab-${item.value}` : ''} onClick={() => selectCategory(item.value)} key={item.value}><Icon /><span><b>{item.label}</b><small>{item.description}</small></span><i aria-label={String(count)}>{count}</i></button> })}</div>
