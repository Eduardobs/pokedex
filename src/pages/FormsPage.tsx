...
export function FormsPage() {
  ...
  const grouped = useMemo(() => {
    ...
    const nationalDex = new Map(
      speciesData?.results.map((species) => [species.name, idFromUrl(species.url)]) ?? [],
    )
      ...
      <section className="page content-width forms-directory-page">
        ...
        <div className="forms-directory-grid">
          {Array.from({ length: 8 }, (_, index) => (
            <div className="directory-form-card skeleton" key={index} />
          ))}
        </div>
      </section>
    )
    ...
    <section className="page content-width forms-directory-page">
      <div className="page-title">
        ...
        <div className="result-count">
          <b>{formatNumber(specialForms.length, language)}</b>
          <span>{t('forms.specialCount')}</span>
        </div>
      </div>
      ...
      <div className="directory-summary">
        ...
        <p>
          {filtered.length === 1
            ? t('forms.resultOne')
            : t('forms.results', { count: filtered.length })}
        </p>
      </div>
      {visible.length ? (
        <div className="forms-directory-grid">
      ...
      {hasMore && (
        <div className="infinite-loader" ref={sentinelRef} role="status" aria-live="polite">
          ...
          <button onClick={loadMore}>{t('forms.loadMore')}</button>
        </div>
      )}
